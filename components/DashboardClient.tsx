...
export default function DashboardClient() {
  ...
  const score = useMemo(() => {
    ...
    const persisted = state.underwritingProfile.scoreResult;
    if (persisted) {
      return {
        overallScore: persisted.composite_score,
        confidenceScore: persisted.completeness.percentage,
        explanation: persisted.knockouts.some((knockout) => knockout.triggered)
          ? "The backend score run flagged underwriting conditions that will likely drive follow-up."
          : "This score and recommendation set were generated by the backend scoring pipeline.",
        concerns: persisted.knockouts.filter((knockout) => knockout.triggered).map((knockout) => knockout.explanation),
        strengths: persisted.pillars
          .filter((pillar) => pillar.score >= 70)
          .map((pillar) => `${pillar.pillar_label} is currently one of the stronger underwriting pillars.`),
        recommendedActions: persisted.recommendations
      };
    }
    const fallback = calculateReadiness(state);
    return {
      ...
      recommendedActions: fallback.recommendedActions.map((action) => ({
        id: action.id,
        title: action.title,
        description: action.description,
        category: action.category,
        estimated_gain: action.expectedScoreImpact,
    ...
    setMessages((current) => {
      ...
      return [
        ...current,
        {
          id: "state-seed",
          role: "assistant",
          content: `Loaded ${state.profile.businessName}. Current readiness is ${score?.overallScore ?? 0}/100 with ${state.documentation.missingDocuments.length} missing documentation items.`
        }
      ];
    });
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-600">
                  {state.underwritingProfile.scoreResult
                    ? "Live underwriting workspace backed by persisted backend computation."
